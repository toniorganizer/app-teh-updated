...
class JenisPendidikanController extends Controller
{
    ...
    public function index(){
        ...
        if($sidebar_data->id_disnaker_kab == 0){
            $datalaporan = DataJenisPendidikan::where('id_disnaker', Auth::user()->email)->where('type','Laporan')->whereNotIn('judul', $excludedNumbers)->paginate(20);
        }else{
            $datalaporan = DataJenisPendidikan::where('id_disnaker', $sidebar_data->id_disnaker_kab)->where('type','Laporan')->whereNotIn('judul', $excludedNumbers)->paginate(20);
        }
        ...
        $lap = DB::table('data_jenis_pendidikans')->join('pemangku_kepentingans', 'pemangku_kepentingans.email_lembaga','=','data_jenis_pendidikans.id_disnaker')->where('role_acc', 1)
    ...
    public function CetakLaporanII($id){
        ...
        if($data_user->icon == 0){
            return redirect('/laporan-ipk-2')->with('success', 'Mohon maaf, silahkan lakukan upload lambang lembaga terlebih dahulu pada menu profile!!!');
        }else{
    ...
    public function editLaporanII(Request $request, $id){
        if($request->id_disnaker){
            if($request->type == 'Lampiran'){
                ...
                $data = DataJenisPendidikan::where('nmr', $id)->Where('id_disnaker', $request->id_disnaker)->where('type','Lampiran')->whereNotIn('judul', $notIn)->first();
            }else{
                $notIn = ['BH & TIDAK TAMAT SD','SD'];
                $data = DataJenisPendidikan::where('nmr', $id)->Where('id_disnaker', $request->id_disnaker)->where('type','Laporan')->whereNotIn('judul', $notIn)->first();
            }
        }
        elseif($request->type == 'Lampiran'){
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            $data = DataJenisPendidikan::where('nmr', $id)->where('id_disnaker', Auth::user()->email)->where('type','Lampiran')->whereNotIn('judul', $notIn)->first();
        }
        else{
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            $data = DataJenisPendidikan::where('nmr', $id)->where('id_disnaker', Auth::user()->email)->where('type','Laporan')->whereNotIn('judul', $notIn)->first();
        }
    ...
    public function updateLaporanII(Request $request, $id){
        if($request->type == 'Laporan'){
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            DataJenisPendidikan::where('nmr', $id)->where('id_disnaker', $request->id_disnaker)->where('type','Laporan')->whereNotIn('judul', $notIn)->update([
            ...
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            DataJenisPendidikan::where('nmr', $id)->where('id_disnaker', $request->id_disnaker)->where('type','Lampiran')->whereNotIn('judul', $notIn)->update([
                'sisa_l' => $request->{'sisa_l'},
                'sisa_p' => $request->{'sisa_p'},
                'terdaftar_l' => $request->{'terdaftar_l'},
                'terdaftar_p' => $request->{'terdaftar_p'},
