...
class lowonganPendidikanController extends Controller
{
    public function index(){
        $data = DataLowonganPendidikan::get();
        ...
        if($aturan->id_disnaker_kab == 0){
            $datalaporan = DataLowonganPendidikan::where('id_disnaker', Auth::user()->email)->where('type','Laporan')->whereNotIn('judul_lp', $excludedNumbers)->paginate(20);
        }else{
            $datalaporan = DataLowonganPendidikan::where('id_disnaker', $aturan->id_disnaker_kab)->where('type','Laporan')->whereNotIn('judul_lp', $excludedNumbers)->paginate(20);
        }
        ...
        $lap = DB::table('data_lowongan_pendidikans')->join('pemangku_kepentingans', 'pemangku_kepentingans.email_lembaga','=','data_lowongan_pendidikans.id_disnaker')->where('role_acc', 1)
    ...
    public function importDataIPK4(Request $request){
     ...
     public function deleteLaporanIV($id){
        DataLowonganPendidikan::where('id_disnaker', $id)->where('type','Laporan')->delete();
     ...
     public function editLaporanIV(Request $request, $id){
        if($request->id_disnaker){
            if($request->type == 'Lampiran'){
                ...
                $data = DataLowonganPendidikan::where('nmr', $id)->where('type','Lampiran')->Where('id_disnaker', $request->id_disnaker)->whereNotIn('judul_lp', $notIn)->first();
            }else{
                $notIn = ['BH & TIDAK TAMAT SD','SD'];
                $data = DataLowonganPendidikan::where('nmr', $id)->where('type','Laporan')->Where('id_disnaker', $request->id_disnaker)->whereNotIn('judul_lp', $notIn)->first();
            }
        }
        elseif($request->type == "Lampiran"){
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            $data = DataLowonganPendidikan::where('nmr', $id)->where('type','Lampiran')->where('id_disnaker', Auth::user()->email)->whereNotIn('judul_lp', $notIn)->first();
        }
        else{
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            $data = DataLowonganPendidikan::where('nmr', $id)->where('type','Laporan')->where('id_disnaker', Auth::user()->email)->whereNotIn('judul_lp', $notIn)->first();
        }
    ...
    public function updateLaporanIV(Request $request, $id){
        if($request->type == "Laporan"){
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            DataLowonganPendidikan::where('nmr', $id)->where('type','Laporan')->where('id_disnaker', $request->id_disnaker)->whereNotIn('judul_lp', $notIn)->update([
            ...
            $notIn = ['BH & TIDAK TAMAT SD','SD'];
            DataLowonganPendidikan::where('nmr', $id)->where('type','Lampiran')->where('id_disnaker', $request->id_disnaker)->whereNotIn('judul_lp', $notIn)->update([
                'sisa_l_lp' => $request->{'sisa_l'},
                'sisa_p_lp' => $request->{'sisa_p'},
                'terdaftar_l_lp' => $request->{'terdaftar_l'},
                'terdaftar_p_lp' => $request->{'terdaftar_p'},
     ...
     public function CetakLaporanIV($id){
        ...
        if($data_user->icon == 0){
            return redirect('/laporan-ipk-4')->with('success', 'Mohon maaf, silahkan lakukan upload lambang lembaga terlebih dahulu pada menu profile!!!');
        }else{
    ...
    public function detailLaporanKabIV($id){
        ...
        $excludedNumbers = ['Sub Total','Total', 'BH & TIDAK TAMAT SD', 'SD', 'SLTP UMUM','SLTP KEJURUAN', 'SETINGKAT SLTP','PENDIDIKAN MENENGAH ATAS','SMK - TEKNOLOGI DAN REKAYASA','SMK - TEKNOLOGI INFORMASI DAN KOMUNIKASI','SMK - KESEHATAN','SMK - SENI, KERAJINAN DAN PARIWISATA','SMK - AGRIBISNIS DAN AGROTEKNOLOGI','SMK - BISNIS DAN MANAJEMEN','SETINGKAT SMU LAINNYA','DIPLOMA I / AKTA I / DIPLOMA II / AKTA II','DIPLOMA III / AKTA III/ AKADEMI/S.MUDA','SARJANA ( S1 )','SARJANA ( S2 )', 'SETINGKAT SLTP', '0'];
        $datalaporan = DataLowonganPendidikan::where('id_disnaker', $id)->where('type','Laporan')->whereNotIn('judul_lp', $excludedNumbers)->paginate(20);
