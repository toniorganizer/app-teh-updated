...
class KelompokJabatanController extends Controller
{
    ...
    public function index(){
        ...
        if($aturan->id_disnaker_kab == 0){
            ...
            $datalaporan = DataKelompokJabatan::where('id_disnaker', $aturan->id_disnaker_kab)->where('type','Laporan')->whereNotIn('judul_kj', $excludedNumbers)->paginate(20);
        }
        ...
        $lap = DB::table('data_kelompok_jabatans')->join('pemangku_kepentingans', 'pemangku_kepentingans.email_lembaga','=','data_kelompok_jabatans.id_disnaker')->where('role_acc', 1)
     ...
     public function editLaporanIII(Request $request, $id){
        if($request->id_disnaker){
            if($request->type == 'Lampiran'){
                ...
                $data = DataKelompokJabatan::where('nmr', $id)->where('type','Lampiran')->Where('id_disnaker', $request->id_disnaker)->whereNotIn('judul_kj', $notIn)->first();
            }else{
                $notIn = ['BH & TIDAK TAMAT SD','SD'];
    ...
    public function updateLaporanIII(Request $request, $id){
     ...
     public function CetakLaporanIII($id){
        ...
        if($data_user->icon == 0){
            return redirect('/laporan-ipk-3')->with('success', 'Mohon maaf, silahkan lakukan upload lambang lembaga terlebih dahulu pada menu profile!!!');
        }else{
            $lambang= $data_user->icon;
        }
        if($id == 'disnaker@gmail.com' || $data->status_lembaga == 3){
            $fileName = 'Laporan-IPK-3-'. $data->nama_lembaga .'.xlsx';
            return Excel::download(new CetakLaporanIIIPusat($id, $lambang), $fileName);
        }elseif($item == null){
