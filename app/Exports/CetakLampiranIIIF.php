...
class CetakLampiranIIIF implements WithDrawings, WithStyles, WithTitle, FromView, WithColumnWidths
{
    private $id;
    private $lambang;
    ...
    public function view(): View
    {
        ...
        $start = 0;
        $end = 9;
        $id_kadis = DataLowonganJabatan::where('id_disnaker', $this->id)->where('type', 'Lampiran')->first();
        if(is_null($id_kadis)){
            $data = DB::table('data_lowongan_jabatans')->join('pemangku_kepentingans', 'pemangku_kepentingans.id_disnaker_kab','=','data_lowongan_jabatans.id_disnaker')->where('email_lembaga', $this->id)->where('type','Lampiran')
        ...
        ->select('judul_lj', 'nmr', 'akhir_l_lj', 'akhir_p_lj')->where('type','Lampiran')
        ->where(function ($query) use ($start, $end) {
            $query->whereBetween('nmr', [$start, $end])
                    ->orWhere('nmr', '4.12');
        })
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN sisa_l_lj ELSE SUM(sisa_l_lj) END AS sisa_l')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN sisa_p_lj ELSE SUM(sisa_p_lj) END AS sisa_p')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN terdaftar_l_lj ELSE SUM(terdaftar_l_lj) END AS terdaftar_l')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN terdaftar_p_lj ELSE SUM(terdaftar_p_lj) END AS terdaftar_p')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN penempatan_l_lj ELSE SUM(penempatan_l_lj) END AS penempatan_l')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN penempatan_p_lj ELSE SUM(penempatan_p_lj) END AS penempatan_p')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN hapus_l_lj ELSE SUM(hapus_l_lj) END AS hapus_l')
        ->selectRaw('CASE WHEN judul_lj = "JUMLAH" THEN hapus_p_lj ELSE SUM(hapus_p_lj) END AS hapus_p')
