...
class CetakLampiranIIIE implements WithDrawings, WithStyles, WithTitle, FromView, WithColumnWidths
{
    ...
    public function view(): View
    {
        ...
        if(is_null($semester)){
            $semester = DataLowonganPendidikan::where('type','Lampiran')->first();
        }else{
        ...
        $start = 1000;
        $end = 7600;
        $id_kadis = DataLowonganPendidikan::where('id_disnaker', $this->id)->where('type', 'Lampiran')->first();
        if(is_null($id_kadis)){
            $data = DB::table('data_lowongan_pendidikans')->join('pemangku_kepentingans', 'pemangku_kepentingans.id_disnaker_kab','=','data_lowongan_pendidikans.id_disnaker')->where('email_lembaga', $this->id)->where('type','Lampiran')
        ...
        ->select('judul_lp', 'nmr', 'akhir_l_lp', 'akhir_p_lp')->where('type','Lampiran')
        ->where(function ($query) use ($start, $end) {
            $query->whereBetween('nmr', [$start, $end])
                    ->orWhere('nmr', '4.11');
        })
