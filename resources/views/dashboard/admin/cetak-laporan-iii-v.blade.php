...
@if($disnaker->email_lembaga == 'disnaker@gmail.com' || $semester->type == 'Lampiran')
    ...
    <tr><td></td><td></td><td colspan="10" style="text-align: center;">IPK III/12 : LOWONGAN KERJA YANG TERDAFTAR, DIPENUHI DAN DIHAPUSKAN</td></tr>
...
@if($lap->judul_lj == 'TOTAL')
    <tr>
        ...
        <td>{{ $lap->hapus_l_s }}</td>
        <td>{{ $lap->hapus_p_s }}</td>
        <td>{{$lap->akhir_l_lj}}</td>
        <td>{{$lap->akhir_p_lj}}</td>
    </tr>
@else
<tr>
    <td>{{$lap->nmr}}</td>
...
<tr>
    <td>{{$item->nmr}}</td>
    <td>{{$item->judul_lj}}</td>
    <td>{{$item->sisa_l_lj}}</td>
    <td>{{$item->sisa_p_lj}}</td>
    <td>{{$item->terdaftar_l_lj}}</td>
    <td>{{$item->terdaftar_p_lj}}</td>
    <td>{{$item->penempatan_l_lj}}</td>
    <td>{{$item->penempatan_p_lj}}</td>
    <td>{{$item->hapus_l_lj}}</td>
    <td>{{$item->hapus_p_lj}}</td>
    <td>{{$item->akhir_l_lj}}</td>
    <td>{{$item->akhir_p_lj}}</td>
</tr>
@endforeach
@endif
</table>
